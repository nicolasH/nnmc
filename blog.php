...
function printTitle($title,$file,$uri){
        ...
        $base = BASE_URI . "blog/";
        if(isset($date)){
           $html.= '<span class="date"><a class="date" href="'.$base.$date['year'].'/">'. $date['year'].'</a> / ';
           $html.= '<a class="date" href="'.$base.$date['year'].'/'.$date['month'].'/">'.$date['month'] .'</a> / ';
           $html.= '<a class="date" href="'.$base.$date['year'].'/'.$date['month'].'/'.$date['day'].'/">'.$date['day'].'</a> </span> ' ;
        }
        if(isset($title)){
                $html = '<div class="title">'.$html;
                $html.= '</span><a href="'.$uri.'" >'.$title.'</a>';
                $html.= "</div>\n";
        }
        return $html;
}
